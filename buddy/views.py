...
def getAuthor(request, pk):
    try:
        ...
        logger.error(f'Author with pk {pk} not found.')
        return Response(status=status.HTTP_404_NOT_FOUND)
    ...
    if request.method == 'GET':
        ...
        if serializer.is_valid():
            serializer.save()
            logger.info(f'Author with pk {pk} updated successfully.')
            return Response(serializer.data)
        logger.warning(f'Failed to update author with pk {pk}: {serializer.errors}')
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)

    elif request.method == 'DELETE':
        author.delete()
        logger.info(f'Author with pk {pk} deleted successfully.')
        return Response(status=status.HTTP_204_NO_CONTENT)
# def getAuthor(request, pk):
